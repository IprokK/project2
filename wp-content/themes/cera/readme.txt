=== Cera ===
Contributors: Themosaurus
Requires at least: 5.0
Tested up to: WordPress 5.8.0
Version: 1.1.8
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Tags: one-column, two-columns, three-columns, left-sidebar, right-sidebar, flexible-header, grid-layout, buddypress, custom-background, custom-colors, custom-logo, custom-header, custom-menu, featured-image-header, featured-images, footer-widgets, full-width-template, microformats, post-formats, sticky-post, threaded-comments, rtl-language-support, translation-ready, theme-options, blog, e-commerce, education, entertainment, food-and-drink, holiday, news, photography, portfolio

== Description ==

Cera is a Dating & Community theme that integrates seamlessly with BuddyPress and WooCommerce. It makes good use of the Customizer, allowing you to build a tailor-made community plateform using custom color, typography and layout options.

== Installation ==

1. In your admin panel, go to Appearance > Themes and click the 'Add New' button.
2. Click Upload and Choose File, then select the theme's .zip file. Click Install Now.
3. Click on the 'Activate' button to use your new theme right away.
4. Navigate to Appearance > Customize in your admin panel and customize to taste.

== Frequently Asked Questions ==

= Does this theme support any plugins? =

Cera includes support for the following plugins:
- AddToAny Share Buttons
- Adrotate
- Author Avatars
- Autocomplete For Relevanssi
- BBP topic count
- bbPress
- BP Profile Search
- BuddyPress
- Buddypress Global Search
- Buddypress Xprofile Custom Fields Type
- Contact Form 7
- Easy Pricing Tables
- Event Tickets
- Event Tickets Plus
- Community Tickets
- Community Events
- Features by WooThemes
- Google Maps Easy
- Gravity Forms
- Grimlock
- Grimlock Animate
- Grimlock Author Avatar
- Grimlock bbPress
- Grimlock BuddyPress
- Grimlock Hero
- Grimlock Hero Animate
- Grimlock Hero Video
- Grimlock Isotope
- Grimlock The Events Calendar
- Grimlock WooCommerce
- Grimlock WooCommerce Subscriptions
- Grimlock WordPress SEO
- If Menu
- Instagram Feed
- Jetpack (Infinite Scroll)
- Kirki
- Lava Bp Post
- Login With Ajax
- MailChimp for WordPress
- Match me for BuddyPress
- Menu Image
- Mutual Buddies
- Ninja Forms
- Our Team by WooThemes
- Projects by WooThemes
- rtMedia for WordPress, BuddyPress and bbPress
- Slider Revolution
- Testimonials by WooThemes
- The Events Calendar
- The Events Calendar PRO
- WooCommerce
- WooCommerce Product Image Flipper
- WooCommerce Quantity Increment
- WooCommerce Subscriptions
- WP PageNavi
- WP Social Login
- WPML
- Yith Infinite Scrolling
- Yith Woocommerce Quick View
- Yith Woocommerce Wishlist
- Yoast SEO

== Copyright ==

Cera is based on Underscores http://underscores.me/, (C) 2012-2015 Automattic, Inc.
Underscores is distributed under the terms of the GNU GPL v2 or later.

Cera bundles the following third-party resources:

Font Awesome icons, Copyright Dave Gandy
License: SIL Open Font License, version 1.1.
Source: http://fontawesome.io

Bootstrap, Copyright (c) 2011.3.27 Twitter, Inc. Copyright (c) 2011.3.27 The Bootstrap Authors
License : The MIT License (MIT)
Source : http://getbootstrap.com/

Slideout, Copyright (c) 2015 Mango
License : The MIT License (MIT)
Source : https://slideout.js.org/

Repaintless, Copyright (c) 2016 Lunar Logic
License : The MIT License (MIT)
Source : https://slideout.js.org/


== Thanks to ==
• Unsplash
• Pexels
• Pexels videos
• Pixabay
• Goodfreephotos
• Flaticon
• Smashicons
• Webalys
• GraphicBurger
• The WordPress Community
